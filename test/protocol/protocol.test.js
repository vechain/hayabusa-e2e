...
describe('POST /accounts/*', function () {
    ...
    beforeAll(async () => {
        ...
        bestBlockNumber = (await Client.raw.getBlock('best', false, false)).body
            ?.number
    })

    it.e2eTest('validate proposers', 'all', async () => {
        const prevBlock = await Client.raw.getBlock(
            bestBlockNumber - 1,
            false,
            true,
        )
        ...
        const bestBlock = await Client.raw.getBlock(
            bestBlockNumber,
            false,
            false,
        )
        ...
        expect(bestBlock.body.proposer, 'Proposer should be equal').toEqual(
            validator,
        )
    })
})
