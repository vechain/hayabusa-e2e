...
func TestStaker_AddValidator(t *testing.T) {
	...
	receipt, _, err := staker.AddValidator(validator.Address, MinStake, config.MinStakingPeriod, true).Receipt(false)
	...
	assert.Equal(t, MinStake, events[0].Stake)
...
func TestStaker_FirstActive_Next(t *testing.T) {
	staker, _, _ := newTestSetupWithActiveValidators(t, 3, true, MinStake)
	...
	assert.Equal(t, MinStake, firstActive.Weight)
	assert.Equal(t, StatusActive, firstActive.Status)
...
func TestStaker_TotalStake(t *testing.T) {
	amount := uint32(3)
	staker, _, _ := newTestSetupWithActiveValidators(t, amount, false, MinStake)
	totalStake, err := staker.TotalStake()
	assert.NoError(t, err)
	expectedStake := new(big.Int).Mul(MinStake, big.NewInt(int64(amount)))
...
func TestStaker_WithdrawValidator(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, withdrawAmount, MinStake)
...
func TestStaker_IncreaseStake(t *testing.T) {
	...
	// increase the stake
	receipt, _, err := staker.Attach(account.PrivateKey).IncreaseStake(id, MinStake).Receipt(false)
	assert.NoError(t, err)
	...
	assert.Equal(t, id, events[0].ValidationID)
	assert.Equal(t, account.Address, events[0].Endorsor)
	assert.Equal(t, MinStake, events[0].Added)
	...
	assert.NoError(t, err)
	assert.Equal(t, validator.Stake, new(big.Int).Mul(MinStake, big.NewInt(2)))
}

func TestStaker_DecreaseStake(t *testing.T) {
	stake := new(big.Int).Mul(MinStake, big.NewInt(2))
	...
	// decrease the stake
	receipt, _, err := staker.Attach(account.PrivateKey).DecreaseStake(id, MinStake).Receipt(false)
	assert.NoError(t, err)
	...
	assert.Equal(t, id, events[0].ValidationID)
	assert.Equal(t, account.Address, events[0].Endorsor)
	assert.Equal(t, MinStake, events[0].Removed)
	...
	validator, err := staker.Get(id)
	assert.NoError(t, err)
	assert.Equal(t, validator.Stake, MinStake)
}
...
func TestStaker_AddDelegation(t *testing.T) {
	...
	receipt, _, err := staker.AddDelegation(id, delegator.Address, MinStake, true, uint8(100)).Receipt(false)
	assert.NoError(t, err)
	...
	assert.NoError(t, err)
	assert.Equal(t, delegation.Stake, MinStake)

	// multiplier of 100 means 1x weight, so the weight is the same as the stake
	expectedWeight := new(big.Int).Mul(MinStake, big.NewInt(2))
...
func TestStaker_UpdateDelegatorAutoRenew(t *testing.T) {
	delegator := devgenesis.DevAccounts()[9]
	staker, config, validationID := newTestSetupWithActiveDelegator(t, 3, true, false, MinStake, delegator)
	ticker := common.NewTicker(staker.Client())
	...
	assert.Equal(t, MinStake, withdraEvents[0].Stake)
}

func newTestSetup(t *testing.T, maxBlockProposers uint32) (*Staker, *hayabusa.Config) {
	t.Helper()
	...
	staker := NewStaker(client, devgenesis.DevAccounts()[0].PrivateKey)
...
func newTestSetupWithActiveValidators(t *testing.T, maxBlockProposers uint32, autoRenew bool, stake *big.Int) (*Staker, *hayabusa.Config, map[thor.Bytes32]devgenesis.DevAccount) {
	t.Helper()
...
func newTestSetupWithActiveDelegator(t *testing.T, maxBlockProposers uint32, validatorAutoRenew, delegatorAutoRenew bool, stake *big.Int, delegator devgenesis.DevAccount) (*Staker, *hayabusa.Config, thor.Bytes32) {
	t.Helper()
	...
	_, _, err = staker.Attach(delegator.PrivateKey).AddDelegation(validationID, delegator.Address, MinStake, delegatorAutoRenew, uint8(100)).Receipt(false)
	assert.NoError(t, err)
...
func findExitedValidator(staker *Staker, validators map[thor.Bytes32]devgenesis.DevAccount) (devgenesis.DevAccount, thor.Bytes32, bool, error) {
	for id, account := range validators {
		validator, err := staker.Get(id)
		if err != nil {
			return devgenesis.DevAccount{}, thor.Bytes32{}, false, err
		}
		if validator.Status == StatusExited {
