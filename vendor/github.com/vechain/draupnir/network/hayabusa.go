...
func (h HayabusaConfig) Apply(genesis *genesis.CustomGenesis) {
...
func CreateHayabusa(config *HayabusaConfig) (*thorclient.Client, func(), error) {
	if config.Nodes < 2 {
	...
	net := NewCustomNetworkWithBranchAndRepo(repo, "release/hayabusa")
	workingDir, ok := os.LookupEnv("THOR_WORKING_DIR")
	if ok {
		net = NewCustomWithRepoAndDownloadPath(repo, workingDir)
	}
