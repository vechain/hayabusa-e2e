...
func TestHayabusaNoForkThenJoinLater(t *testing.T) {
	...
	authority1 := thorgenesis.DevAccounts()[1]
	authority2 := thorgenesis.DevAccounts()[2]
	authority3 := thorgenesis.DevAccounts()[0]

	block := config.ForkBlock
	staker := builtins.NewStaker(client, authority1.PrivateKey)
	assert.NoError(t, staker.WaitForFork(block))
	ticker := common.NewTicker(client)

	id1 := addValidator(t, staker, authority1.PrivateKey, authority1.Address, false, config.MinStakingPeriod)

	firstQueued, _, err := staker.FirstQueued()
	assert.NoError(t, err)
	assert.Equal(t, firstQueued.Endorsor, &authority1.Address)
	...
	id2 := addValidator(t, staker, authority2.PrivateKey, authority2.Address, false, config.MinStakingPeriod)
...
func TestHayabusaFullFlowJoinQueuedCooldownExit(t *testing.T) {
	...
	authority1 := thorgenesis.DevAccounts()[0]
	authority2 := thorgenesis.DevAccounts()[1]
	authority3 := thorgenesis.DevAccounts()[2]
	...
	id1 := addValidator(t, staker, authority1.PrivateKey, authority1.Address, false, config.MinStakingPeriod)
	id2 := addValidator(t, staker, authority2.PrivateKey, authority2.Address, false, config.MinStakingPeriod)
	id3 := addValidator(t, staker, authority3.PrivateKey, authority3.Address, true, config.MinStakingPeriod)
	...
	validatorWithdraw(t, staker, authority1.PrivateKey, id1)
}
...
func TestHayabusaQueuedAndThenEnter(t *testing.T) {
	...
	authority2 := thorgenesis.DevAccounts()[1]
	authority3 := thorgenesis.DevAccounts()[2]
	authority4 := thorgenesis.DevAccounts()[3]
	authority5 := thorgenesis.DevAccounts()[4]
	...
	id1 := addValidator(t, staker, authority1.PrivateKey, authority1.Address, true, config.MinStakingPeriod)
	id2 := addValidator(t, staker, authority2.PrivateKey, authority2.Address, true, config.MinStakingPeriod)
	id3 := addValidator(t, staker, authority3.PrivateKey, authority3.Address, true, config.MinStakingPeriod)
	id4 := addValidator(t, staker, authority4.PrivateKey, authority4.Address, true, config.MinStakingPeriod)
	...
	id5 := addValidatorWithStake(t, staker, authority5.PrivateKey, authority5.Address, false, stake, config.MinStakingPeriod)
	block += config.EpochLength
	assertValidatorStatus(t, staker, id1, builtins.StatusActive, block)
	assertValidatorStatus(t, staker, id2, builtins.StatusActive, block)
	assertValidatorStatus(t, staker, id3, builtins.StatusActive, block)
	...
	receipt, _, err := staker.Attach(authority3.PrivateKey).UpdateAutoRenew(id3, false).Receipt(false)
	assert.NoError(t, err)
	assert.Equal(t, staker.Address().String(), receipt.Outputs[0].Events[0].Address.String())
	assert.Equal(t, authority3.Address.Bytes(), receipt.Outputs[0].Events[0].Topics[1].Bytes()[12:])
