...
func runTestStargateSingleDelegator(t *testing.T) error {
	...
	completed, err := staker.GetValidationPeriodDetails(validationID)
	require.NoError(t, err)
	...
	for i := start; i <= end; i++ {
		reward, err := staker.GetDelegatorRewards(validationID, i)
		require.NoError(t, err)
...
func runTestStargateDelegatorFlowStakeAndClaimAutoRenewOff(t *testing.T) error {
	...
	assert.NoError(t, err)
	assert.True(t, del.Locked)

	stakerRewards, err := staker.GetDelegatorRewards(validationID, 3)
	assert.NoError(t, err)
...
func runTestStargateDelegatorFlowStakeAndClaimAutoRenewOnAndOff(t *testing.T) error {
	...
	totalRewards, err := staker.GetDelegatorRewards(validationID, 3)
	assert.NoError(t, err)
...
func setStargate(t *testing.T, staker *builtin.Staker) *stargate.Stargate {
	...
	require.NoError(t, err)
	key := thor.BytesToBytes32([]byte(hayabusa.ParamsStargateKey))
