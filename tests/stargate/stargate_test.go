...
func runTestStargateDelegatorFlowStakeAndClaimAutoRenewOff(t *testing.T) error {
	...
	require.NoError(t, err)
	assert.Equal(t, expectedCompletedPeriods, completed.CompletedPeriods)
	delegationPeriod, err := staker.GetDelegationPeriodDetails(delegationID)
	require.NoError(t, err)
	assert.Equal(t, 3, int(delegationPeriod.StartPeriod))
...
func runTestStargateDelegatorFlowStakeAndClaimAutoRenewOnAndOff(t *testing.T) error {
	...
	require.NoError(t, err)
	assert.Equal(t, expectedCompletedPeriods, completed.CompletedPeriods)
	delegationPeroid, err := staker.GetDelegationPeriodDetails(delegationID)
	require.NoError(t, err)
	assert.Equal(t, 3, int(delegationPeroid.StartPeriod))
